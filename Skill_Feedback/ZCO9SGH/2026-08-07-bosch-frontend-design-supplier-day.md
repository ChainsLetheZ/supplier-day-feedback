...
## Follow-up Correction (2026-08-07)

- 移除期待选项区域的嵌套滚动，改由问卷主滚动容器统一承载，避免移动端内容被截断且无法勾选。
- 为主滚动容器及步骤内容补充 `min-height: 0`，确保矩阵题最后一行可以滚动到并选择。
- 按 FROK 控件规范将按钮、输入框、下拉框和文本框圆角设为 0；按钮触摸高度保持至少 44px，焦点轮廓调整为 3px。
- 删除重复的活动标题和“步骤 Step x / 5”文字，改为 Bosch Logo 下方的无文字进度条，减少移动端页头占用。
